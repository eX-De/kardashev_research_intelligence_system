...
- `compute_token.txt`: required random service token shared only by the app and internal compute service.
...
node -e "console.log(require('crypto').randomBytes(32).toString('base64url'))" | Set-Content -NoNewline secrets/compute_token.txt
```
