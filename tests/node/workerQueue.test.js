...
test("ordinary enqueue creates only worker task state and queued outbox event transactionally", async () => {
  await withWorkerQueuePool(async (fake) => {
    ...
    assert.equal(result.worker_job.concurrency_group, "daily");
    assert.equal(result.worker_job.concurrency_key, "global:daily");
    assert.equal(result.worker_job.policy_version, 3);
