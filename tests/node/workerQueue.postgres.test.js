...
test("ordinary enqueue does not create job_runs while daily enqueue remains linked", {
  ...
  try {
    ...
    const mirror = (await client.query(`
      ...
      VALUES ('sync-obsidian', 'completed', NOW(), 'legacy mirror', NOW(), '{"worker_job":true}')
      RETURNING id
    `)).rows[0];
    const linkedWorker = (await client.query(`
      ...
      VALUES ($1, 'sync-obsidian', 'completed', '{}', NOW(), NOW())
      RETURNING id
    `, [mirror.id])).rows[0];
    const orphan = (await client.query(`
      INSERT INTO job_runs(job_type, status, started_at, message, heartbeat_at, meta_json)
      VALUES ('fetch-arxiv', 'failed', NOW(), 'orphan legacy history', NOW(), '{"worker_job":true}')
      RETURNING id
    `)).rows[0];
