...
CREATE TABLE IF NOT EXISTS paper_chunk_embeddings (
  ...
  embedding_json TEXT NOT NULL,
  created_at TEXT NOT NULL,
  PRIMARY KEY(paper_chunk_id, model)
);
...
CREATE TABLE IF NOT EXISTS worker_instances (
  ...
  current_job_id INTEGER REFERENCES worker_jobs(id) ON DELETE SET NULL,
  pid INTEGER,
  meta_json TEXT NOT NULL DEFAULT '{}'
);
...
CREATE TABLE IF NOT EXISTS worker_runtime_policy (
  singleton_id SMALLINT PRIMARY KEY CHECK (singleton_id = 1),
  revision BIGINT NOT NULL CHECK (revision >= 1),
  worker_process_count INTEGER NOT NULL CHECK (worker_process_count BETWEEN 1 AND 16),
  global_llm_request_concurrency INTEGER NOT NULL CHECK (global_llm_request_concurrency BETWEEN 1 AND 64),
  global_embedding_request_concurrency INTEGER NOT NULL CHECK (global_embedding_request_concurrency BETWEEN 1 AND 64),
  embedding_concurrency INTEGER NOT NULL CHECK (embedding_concurrency BETWEEN 1 AND 32),
  project_judgment_concurrency INTEGER NOT NULL CHECK (project_judgment_concurrency BETWEEN 1 AND 8),
  project_chat_profile_concurrency INTEGER NOT NULL CHECK (project_chat_profile_concurrency BETWEEN 1 AND 8),
  updated_at TEXT NOT NULL,
  CHECK (embedding_concurrency <= global_embedding_request_concurrency),
  CHECK (project_judgment_concurrency <= global_llm_request_concurrency),
  CHECK (project_chat_profile_concurrency <= global_llm_request_concurrency)
);

CREATE TABLE IF NOT EXISTS worker_group_limit_overrides (
  concurrency_group TEXT PRIMARY KEY,
  max_running INTEGER NULL CHECK (max_running IS NULL OR max_running >= 1),
...
CREATE TABLE IF NOT EXISTS project_paper_recommendations (
  project_id INTEGER NOT NULL REFERENCES research_projects(id) ON DELETE CASCADE,
  paper_id INTEGER NOT NULL REFERENCES papers(id) ON DELETE CASCADE,
  source_arxiv_paper_id INTEGER REFERENCES arxiv_papers(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS paper_reader_messages (
  id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  library_paper_id INTEGER NOT NULL REFERENCES papers(id) ON DELETE CASCADE,
  role TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS paper_reader_references (
  paper_id INTEGER NOT NULL REFERENCES papers(id) ON DELETE CASCADE,
  reference_paper_id INTEGER NOT NULL REFERENCES papers(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS artifact_chunk_embeddings (
  artifact_chunk_id INTEGER NOT NULL REFERENCES artifact_chunks(id) ON DELETE CASCADE,
  model TEXT NOT NULL,
  embedding_json TEXT NOT NULL,
  created_at TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_paper_reader_messages_library ON paper_reader_messages(library_paper_id, id);
CREATE INDEX IF NOT EXISTS idx_paper_reader_references ON paper_reader_references(paper_id, position);
...
CREATE INDEX IF NOT EXISTS idx_artifact_chunks_artifact ON artifact_chunks(artifact_id, chunk_index);
CREATE INDEX IF NOT EXISTS idx_artifact_chunk_embeddings_model ON artifact_chunk_embeddings(model, artifact_chunk_id);
CREATE INDEX IF NOT EXISTS idx_paper_embeddings_model ON paper_embeddings(model, paper_id);
CREATE INDEX IF NOT EXISTS idx_paper_chunk_embeddings_model ON paper_chunk_embeddings(model, paper_chunk_id);
"""
...
REQUIRED_INDEXES = {
    ...
    "idx_paper_reader_messages_library",
    "idx_paper_reader_references",
    "idx_artifacts_scope",
    "idx_artifacts_type",
    "idx_artifact_chunks_artifact",
    "idx_artifact_chunk_embeddings_model",
    "idx_paper_embeddings_model",
    "idx_paper_chunk_embeddings_model",
}
...
REQUIRED_COLUMNS = {
    "worker_runtime_policy": {
        "singleton_id", "revision", "worker_process_count",
        "global_llm_request_concurrency", "global_embedding_request_concurrency",
        "embedding_concurrency", "project_judgment_concurrency",
        "project_chat_profile_concurrency", "updated_at",
    },
    "worker_group_limit_overrides": {
    ...
    "paper_reader_messages": {"context_json", "library_paper_id"},
    "job_runs": {
        "pid",
        "heartbeat_at",
    },
...
REQUIRED_FOREIGN_KEYS = {
    ("user_feedback", "paper_id"): "papers",
    ("project_papers", "paper_id"): "papers",
    ("project_paper_recommendations", "paper_id"): "papers",
}
...
def _seed_worker_runtime_policy(conn: Any) -> dict[str, Any]:
    ...
    defaults = {
        "worker_process_count": ("WORKER_PROCESS_COUNT", 1, 1, 16),
        "global_llm_request_concurrency": ("GLOBAL_LLM_REQUEST_CONCURRENCY", 4, 1, 64),
        "global_embedding_request_concurrency": ("GLOBAL_EMBEDDING_REQUEST_CONCURRENCY", 4, 1, 64),
        "embedding_concurrency": ("EMBEDDING_CONCURRENCY", 2, 1, 32),
        "project_judgment_concurrency": ("PROJECT_JUDGMENT_CONCURRENCY", 3, 1, 8),
        "project_chat_profile_concurrency": ("PROJECT_CHAT_PROFILE_CONCURRENCY", 2, 1, 8),
    }
    stored_rows = conn.execute(
        "SELECT key, value_json FROM app_settings WHERE key = ANY(?)",
        (list(defaults),),
    ).fetchall()
    stored = {str(row["key"]): from_json(row["value_json"], None) for row in stored_rows}
    values: dict[str, int] = {}
    corrections: list[str] = []
    for field, (env_name, fallback, minimum, maximum) in defaults.items():
        raw = stored.get(field, env_value(env_name, str(fallback)))
        try:
            parsed = int(raw)
        except (TypeError, ValueError):
            parsed = fallback
            corrections.append(f"{field}: invalid {raw!r} -> {parsed}")
        bounded = min(maximum, max(minimum, parsed))
        if bounded != parsed:
            corrections.append(f"{field}: {parsed} -> {bounded}")
        values[field] = bounded
    for local_field, global_field in (
        ("embedding_concurrency", "global_embedding_request_concurrency"),
        ("project_judgment_concurrency", "global_llm_request_concurrency"),
        ("project_chat_profile_concurrency", "global_llm_request_concurrency"),
    ):
        if values[local_field] > values[global_field]:
            corrections.append(f"{local_field}: {values[local_field]} -> {values[global_field]}")
            values[local_field] = values[global_field]
    seeded_at = utc_now()
    conn.execute(
        """INSERT INTO worker_runtime_policy(
               singleton_id, revision, worker_process_count,
               global_llm_request_concurrency, global_embedding_request_concurrency,
               embedding_concurrency, project_judgment_concurrency,
               project_chat_profile_concurrency, updated_at
        ...
        (
            values["worker_process_count"], values["global_llm_request_concurrency"],
            values["global_embedding_request_concurrency"], values["embedding_concurrency"],
...
def _postgres_schema_current(conn) -> bool:
    ...
    for table, required_columns in REQUIRED_COLUMNS.items():
        if not required_columns.issubset(_postgres_columns(conn, table)):
            return False
    for (table, column), target_table in REQUIRED_FOREIGN_KEYS.items():
        if _postgres_foreign_key_target(conn, table, column) != target_table:
            return False
...
def _postgres_foreign_key_target(conn, table: str, column: str) -> str:
    row = conn.execute(
        """
        SELECT foreign_table.table_name AS target_table
        FROM information_schema.table_constraints constraint_row
        JOIN information_schema.key_column_usage local_column
          ON local_column.constraint_schema = constraint_row.constraint_schema
         AND local_column.constraint_name = constraint_row.constraint_name
        JOIN information_schema.constraint_column_usage foreign_table
          ON foreign_table.constraint_schema = constraint_row.constraint_schema
         AND foreign_table.constraint_name = constraint_row.constraint_name
        WHERE constraint_row.constraint_schema = current_schema()
          AND constraint_row.constraint_type = 'FOREIGN KEY'
          AND constraint_row.table_name = ?
          AND local_column.column_name = ?
        LIMIT 1
        """,
        (table, column),
    ).fetchone()
    return str(row["target_table"]) if row else ""
...
def _migrate_user_paper_relations_to_canonical(conn) -> None:
    """Move every user-visible paper relationship onto ``papers.id``.

    Discovery matches and judgments intentionally retain ``arxiv_papers.id``.
    A recommendation is the admission boundary: once it exists, the paper is a
    durable user asset, including after it is discarded.
    """
    relation_tables = (
        "project_papers",
        "project_paper_recommendations",
    ...
    legacy_tables = [
        ...
        if _postgres_foreign_key_target(conn, table, "paper_id") == "arxiv_papers"
    ]
    if not legacy_tables:
        return

    # Auto-matched candidates are discovery output, not durable project links.
    if "project_papers" in legacy_tables:
        conn.execute(
            """
            DELETE FROM project_papers
            WHERE relation = 'candidate'
              AND note = 'auto_matched_by_project_context'
            """
        )
    if "project_paper_recommendations" in legacy_tables:
        conn.execute(
            """
            UPDATE project_paper_recommendations
            SET source_arxiv_paper_id = paper_id
            WHERE source_arxiv_paper_id IS NULL
            """
        )
    ...
    for table in legacy_tables:
        legacy_ids.update(
            int(row["paper_id"])
            for row in conn.execute(f"SELECT DISTINCT paper_id FROM {table}").fetchall()
        )
    ...
    for legacy_id in sorted(legacy_ids):
        legacy_paper = conn.execute("SELECT * FROM arxiv_papers WHERE id = ?", (legacy_id,)).fetchone()
        if not legacy_paper:
            raise RuntimeError(f"Cannot migrate arXiv paper {legacy_id} to the canonical library")
        canonical_id = upsert_paper_from_arxiv(conn, legacy_paper, library_status="candidate")
        canonical_ids[legacy_id] = int(canonical_id)
    ...
    for table in legacy_tables:
        ...
        for legacy_id, canonical_id in canonical_ids.items():
            conn.execute(
                f"UPDATE {table} SET canonical_paper_id = ? WHERE paper_id = ?",
                (canonical_id, legacy_id),
            )
        unresolved = conn.execute(
            f"SELECT COUNT(*) AS count FROM {table} WHERE canonical_paper_id IS NULL"
        ).fetchone()
        if unresolved and int(unresolved["count"] or 0):
            raise RuntimeError(f"Cannot migrate every {table}.paper_id to papers.id")

        if table in {"project_papers", "project_paper_recommendations"}:
            conn.execute(
                f"""
                DELETE FROM {table}
                WHERE ctid IN (
                  ...
                  FROM (
                    SELECT
                      ctid AS row_id,
                      ROW_NUMBER() OVER (
                        PARTITION BY project_id, canonical_paper_id
                ...
                WHERE ctid IN (
                  ...
                  FROM (
                    SELECT
                      ctid AS row_id,
                      ROW_NUMBER() OVER (
                        PARTITION BY canonical_paper_id, status
        ...
        conn.execute(f"ALTER TABLE {table} DROP COLUMN paper_id CASCADE")
        conn.execute(f"ALTER TABLE {table} RENAME COLUMN canonical_paper_id TO paper_id")
        conn.execute(f"ALTER TABLE {table} ALTER COLUMN paper_id SET NOT NULL")
        conn.execute(
            f"ALTER TABLE {table} ADD CONSTRAINT {table}_paper_id_fkey "
            "FOREIGN KEY (paper_id) REFERENCES papers(id) ON DELETE CASCADE"
        )
        if table in {"project_papers", "project_paper_recommendations"}:
            conn.execute(
                f"ALTER TABLE {table} ADD CONSTRAINT {table}_pkey PRIMARY KEY (project_id, paper_id)"
            )
...
def _migrate_postgres_db(conn) -> None:
    ...
    column_migrations = {
        ...
        "worker_instances": {
            ...
            "heartbeat_at": "heartbeat_at TEXT NOT NULL DEFAULT ''",
            "current_job_id": "current_job_id INTEGER REFERENCES worker_jobs(id) ON DELETE SET NULL",
            "pid": "pid INTEGER",
        ...
        "paper_reader_messages": {
            ...
            "library_paper_id": "library_paper_id INTEGER REFERENCES papers(id) ON DELETE CASCADE",
        },
        "matches": {
    ...
    tables = _postgres_names(conn, "table")
    legacy_reference_ids: set[int] = set()
    if "paper_reader_reference_papers" in tables:
        legacy_reference_ids = {
            int(row["paper_id"])
            for row in conn.execute(
                """
                SELECT paper_id FROM paper_reader_reference_papers
                UNION
                SELECT reference_paper_id AS paper_id FROM paper_reader_reference_papers
                """
            ).fetchall()
        }

    has_legacy_message_id = "paper_id" in _postgres_columns(conn, "paper_reader_messages")
    unresolved_message_ids: set[int] = set()
    if has_legacy_message_id:
        _postgres_drop_not_null_if_present(conn, "paper_reader_messages", "paper_id")
        conn.execute(
            """
            UPDATE paper_reader_messages message
            SET library_paper_id = artifact.scope_id
            FROM artifacts artifact
            WHERE message.library_paper_id IS NULL
              AND artifact.scope_type = 'paper'
              AND artifact.artifact_type = 'paper_report'
              AND artifact.scope_id IS NOT NULL
              AND COALESCE(
                CASE WHEN (artifact.content_json::jsonb ->> 'legacy_arxiv_paper_id') ~ '^[0-9]+$'
                  THEN (artifact.content_json::jsonb ->> 'legacy_arxiv_paper_id')::bigint END,
                CASE WHEN (artifact.source_json::jsonb ->> 'legacy_arxiv_paper_id') ~ '^[0-9]+$'
                  THEN (artifact.source_json::jsonb ->> 'legacy_arxiv_paper_id')::bigint END
              ) = message.paper_id
            """
        )
        conn.execute(
            """
            UPDATE paper_reader_messages message
            SET library_paper_id = (
              SELECT library_paper.id
              FROM arxiv_papers legacy_paper
              JOIN papers library_paper ON (
                library_paper.arxiv_id = legacy_paper.arxiv_id
                OR EXISTS (
                  SELECT 1 FROM paper_sources source
                  WHERE source.paper_id = library_paper.id
                    AND source.source_identifier = legacy_paper.arxiv_id
                )
              )
              WHERE legacy_paper.id = message.paper_id
              ORDER BY library_paper.id
              LIMIT 1
            )
            WHERE message.library_paper_id IS NULL
            """
        ...
        unresolved_message_ids = {
            ...
            for row in conn.execute(
                ...
                WHERE library_paper_id IS NULL AND paper_id IS NOT NULL
                """
            ).fetchall()
        }
    legacy_ids_to_promote = sorted(unresolved_message_ids | legacy_reference_ids)
    if legacy_ids_to_promote:
        from .papers import promote_arxiv_paper_to_library

        for legacy_paper_id in legacy_ids_to_promote:
            promote_arxiv_paper_to_library(conn, legacy_paper_id, library_status="saved")
        if has_legacy_message_id:
            conn.execute(
                """
                UPDATE paper_reader_messages message
    ...
    unresolved = conn.execute(
        "SELECT COUNT(*) AS count FROM paper_reader_messages WHERE library_paper_id IS NULL"
    ).fetchone()
    if unresolved and int(unresolved["count"] or 0):
        raise RuntimeError("Cannot migrate reader messages without a canonical paper id")
    conn.execute("ALTER TABLE paper_reader_messages ALTER COLUMN library_paper_id SET NOT NULL")
    if has_legacy_message_id:
        conn.execute("ALTER TABLE paper_reader_messages DROP COLUMN paper_id")
    ...
    if "paper_reader_reference_papers" in tables:
        conn.execute(
            ...
            SELECT DISTINCT
              subject_paper.id,
              reference_paper.id,
              legacy_reference.position,
              legacy_reference.created_at,
              legacy_reference.updated_at
            FROM paper_reader_reference_papers legacy_reference
            JOIN arxiv_papers subject_legacy ON subject_legacy.id = legacy_reference.paper_id
            JOIN LATERAL (
              SELECT paper.id
              FROM papers paper
              WHERE paper.arxiv_id = subject_legacy.arxiv_id
                 OR EXISTS (
                   SELECT 1 FROM paper_sources source
                   WHERE source.paper_id = paper.id
                     AND source.source_identifier = subject_legacy.arxiv_id
                 )
              ORDER BY paper.id
              LIMIT 1
            ) subject_paper ON TRUE
            JOIN arxiv_papers reference_legacy ON reference_legacy.id = legacy_reference.reference_paper_id
            JOIN LATERAL (
              SELECT paper.id
              FROM papers paper
              WHERE paper.arxiv_id = reference_legacy.arxiv_id
                 OR EXISTS (
                   SELECT 1 FROM paper_sources source
                   WHERE source.paper_id = paper.id
                     AND source.source_identifier = reference_legacy.arxiv_id
                 )
              ORDER BY paper.id
              LIMIT 1
            ) reference_paper ON TRUE
            ON CONFLICT(paper_id, reference_paper_id) DO UPDATE SET
              position = excluded.position,
              updated_at = excluded.updated_at
            """
        )
        conn.execute("DROP TABLE paper_reader_reference_papers")

    conn.execute(
        """
        UPDATE artifacts
        SET content_json = (
              (content_json::jsonb - 'legacy_arxiv_paper_id')
              || jsonb_build_object('paper_id', scope_id)
            )::text,
            source_json = (
              (source_json::jsonb - 'legacy_arxiv_paper_id')
              || jsonb_build_object('source_key', 'paper_report:' || scope_id::text)
            )::text
    ...
    unique_indexes = [
        ...
        "CREATE UNIQUE INDEX IF NOT EXISTS idx_artifact_chunks_unique ON artifact_chunks(artifact_id, chunk_index)",
    ]
